...
def get_fear_greed(limit=120):
    data = safe_get("https://api.alternative.me/fng/", params={"limit": limit, "format": "json"})
    rows = data["data"]
...
with c4:
    small_card(
        "近30日波动（Proxy: RV30）",
        f"{float(rv30.dropna().iloc[-1]['rv30'])*100:.1f}%" if rv30["rv30"].notna().any() else "N/A",
        "用于替代 IV 趋势展示"
    )
...
st.caption("说明：这是免本地部署的版本，默认用 RV30/RV90 作为波动率与 IV 的稳定替代指标。你需要更接近截图的“期权 IV/DVOL 指数”，后续再接 Deribit 期权接口即可。")
